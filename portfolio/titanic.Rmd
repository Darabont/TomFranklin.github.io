...
gg <- gg + geom_text(data=filter(df, country=="Germany"),
                     ...
                     color="#9fb059", size=3, vjust=-2, fontface="bold", family="Bliss Pro")
gg <- gg + geom_text(data=filter(df, country=="Germany"),
                     aes(x=ages_18_to_34, y=country, label="Ages 18-34"),
                     color="#edae52", size=3, vjust=-2, fontface="bold", family="Bliss Pro")
# text above points
gg <- gg + geom_text(data=df, aes(x=ages_35, y=country, label=percent_first(ages_35)),
                     color="#9fb059", size=2.75, vjust=2.5, family="Bliss Pro")
gg <- gg + geom_text(data=df, color="#edae52", size=2.75, vjust=2.5, family="Bliss Pro",
...
gg <- gg + geom_text(data=df, aes(label=diff, y=country, x=1.1125), fontface="bold", size=3, family="Bliss Pro")
gg <- gg + geom_text(data=filter(df, country=="Germany"), aes(x=1.1125, y=country, label="DIFF"),
                     color="#7a7d7e", size=3.1, vjust=-2, fontface="bold", family="Bliss Pro")
...
gg <- gg + labs(x=NULL, y=NULL, title="The social media age gap",
                ...
                caption="@mcrmetrics")
gg <- gg + theme_bw(base_family="Bliss Pro")
